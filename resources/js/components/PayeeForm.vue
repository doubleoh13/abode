...
const props = defineProps<{ payee: Payee | null }>();
...
async function save(): Promise<void> {
    ...
    try {
        if (props.payee) {
            await axios.put(`/api/v1/financial/payees/${props.payee.id}`, { name: form.name });
        } else {
            await axios.post('/api/v1/financial/payees', { name: form.name });
        }
