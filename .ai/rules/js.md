...
## Displayed balances end today; future-dated lines live in an Upcoming band
Recurring schedules materialize transactions up to the lead window ahead, so the SPA never lets a future line into a balance or a running total by default. AccountPage passes as_of/to = today (throughToday(), clamped by the selected period) to balances, the register and period-totals, and fetches postings dated from tomorrow separately into upcomingPostings; registerRows prepends an {kind:'upcoming'} divider row and the future postings flagged upcoming (opacity-60, no running balance shown). JournalPage does the same with transactions: the main list gets to = today unless the user set a To filter, and the band is fetched only while To is empty. Dates come from journal.ts localToday()/localTomorrow()/isoDate() — never new Date().toISOString(), which is UTC and can be tomorrow in the evening. The balance sheet already defaults its as-of to today.
