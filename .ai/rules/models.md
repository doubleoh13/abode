...
name stores the leaf segment only; the full path ("Expenses:food:dining-out") is derived — the top-level prefix comes from account_type (AccountType::pathPrefix, capitalized: Assets/Liabilities/Income/Expenses/Equity), never from a root row. Trees are small: load and build in memory, no path columns or nested sets.
